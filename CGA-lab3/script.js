// Получение ссылок на холсты из элементов canvas в HTML
...
// Получение ссылок на слайдеры и кнопку загрузки из HTML
...
window.onload = async () => {
  // Установка размеров холстов в соответствии с их размерами в браузере
...
// Асинхронная функция для изменения изображения на холсте в зависимости от параметров
...
// Функция для обработки изображения и создания гистограммы
...
// Функции для преобразования цветов из RGB в HSV и обратно
function HSVtoRGB(h, s, v) {  /* Логика преобразования HSV в RGB */
  var r, g, b, i, f, p, q, t;
...
function RGBtoHSV(r, g, b) {  /* Логика преобразования RGB в HSV */
  if (arguments.length === 1) {
    (g = r.g), (b = r.b), (r = r.r);
  }
...
// Функция для отображения изображения с учётом пропорций
